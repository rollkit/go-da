...
func BasicDATest(t *testing.T, da da.DA) {
	...
	id1, proof1, err := da.Submit([]Blob{msg1}, 0)
	assert.NoError(t, err)
	assert.NotEmpty(t, id1)
	assert.NotEmpty(t, proof1)

	id2, proof2, err := da.Submit([]Blob{msg2}, 0)
	assert.NoError(t, err)
	assert.NotEmpty(t, id2)
	assert.NotEmpty(t, proof2)

	id3, proof3, err := da.Submit([]Blob{msg1}, 0)
	assert.NoError(t, err)
...
func GetIDsTest(t *testing.T, da da.DA) {
	...
	ids, proofs, err := da.Submit(msgs, 0)
	assert.NoError(t, err)
...
func ConcurrentReadWriteTest(t *testing.T, da da.DA) {
	...
	go func() {
		defer wg.Done()
		for i := uint64(1); i <= 100; i++ {
			_, _, err := da.Submit([][]byte{[]byte("test")}, 0)
			assert.NoError(t, err)
		}
	}()
